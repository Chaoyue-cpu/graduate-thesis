...
在完成基础位姿图构建以及全局几何先验约束建模后，本文进一步引入局部结构观测提供的辅助几何信息，对位姿图优化模型进行补充。与全局长度约束和全局角度约束主要描述巷道整体结构关系不同，本节主要考虑两类局部辅助几何约束，即地面平面辅助约束和侧壁方向辅助约束。其中，地面平面辅助约束基于巷道底板在局部范围内通常可近似视为同一平面的假设，为关键帧位姿提供平面参考；侧壁方向辅助约束则利用侧壁方向观测补充转弯区域或局部结构变化区域中的方向信息。两类约束共同用于丰富位姿图中的局部几何观测信息，并为后续联合优化提供补充约束来源。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\linewidth]{figures/图 3.10清晰版.png}
...
真实地下矿井数据集采集于陕西大海则煤矿井下巷道，由地面移动机器人搭载 Ouster OS1-128 激光雷达完成数据采集，采样频率为 10 Hz，整个采集轨迹长度为 1641.81 m，采集时长为 11.47 min。该数据集具有巷道狭长、结构重复和长距离连续延伸等特点，可用于检验本文方法在真实矿井场景中的适用性。由于该场景难以获得逐帧高精度真值轨迹，本文采用人工后处理构建的高精度参考地图作为评估基准，并基于地图配准结果对不同方法生成的点云地图进行比较。实验中主要从地图整体几何一致性和局部结构保持能力两个方面对建图结果进行分析。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\linewidth]{figures/图 3.12 Gazebo 仿真数据集典型场景.png}
...
当前主流开源激光建图方法包括 LOAM、Cartographer、LEGO\_LOAM 和 HDL-Graph-SLAM 等。其中，Cartographer 采用子图优化框架，在二维激光建图中应用较为广泛，但其三维建图流程与本文基于关键帧位姿图和几何约束的建模方式差异较大；LEGO\_LOAM 与 HDL-Graph-SLAM 均面向三维激光雷达建图任务，前者具有较好的轻量化特征，后者则采用图优化框架并可引入地面等约束。考虑到本文研究的是三维激光雷达矿井巷道建图问题，本文在 3 个数据集上统一选取 LEGO\_LOAM、HDL-Graph-SLAM 与 ours 进行对比。

评价方式方面，3 个数据集均具有参考地图，因此本文统一采用 MapEval 对地图质量进行评价，并通过误差图和地图量化指标比较不同方法的全局几何一致性与局部结构保持能力。其中，真实地下矿井数据集缺少逐帧真值轨迹，因此 MapEval 结果作为其主要评价依据；Bruceton Mine 数据集和 Gazebo 仿真数据集则在此基础上进一步采用第 2 章所述绝对位姿误差指标，对估计轨迹精度进行补充定量分析。

\subsection{基于 MapEval 的地图质量评价}

为了统一比较 3 个数据集上的建图质量，本文首先给出基于 MapEval 的地图评价结果。MapEval 结果包括两部分：其一为地图可视化结果，用于直观展示参考地图与重建地图之间的对应关系及误差空间分布；其二为地图量化指标，用于定量比较不同方法在全局结构一致性和局部几何保持方面的性能。

对于真实地下矿井数据集，由于缺少真值轨迹，首先给出参考地图与重建地图的叠加结果，以直观展示不同方法在长距离连续巷道中的整体结构对齐情况。图~\ref{fig:realmine_traj_compare} 建议重点突出长直巷道、转弯连接区域以及结构重复区域的差异，从而更直观地体现不同方法在全局一致性和局部结构保持方面的表现。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\linewidth]{figures/图 3.13真实地下矿井数据集上的地图对比结果-2.pdf}
    \caption{真实地下矿井数据集上的 MapEval 误差图}
    \label{fig:realmine_traj_compare}
\end{figure}

在此基础上，进一步给出 Bruceton Mine 数据集和 Gazebo 仿真数据集上的 MapEval 误差图。真实地下矿井数据集上的 MapEval 误差图已在图~\ref{fig:realmine_traj_compare} 中给出。为便于横向比较，各组误差图均按统一顺序排列：从左到右依次为 HDL-Graph-SLAM、LEGO\_LOAM 和 ours，并展示其相对于参考地图的 raw error map 或 inlier error map。颜色由蓝到红表示误差逐渐增大，通过比较不同方法的误差分布范围、热点区域及连续性，可直观反映其地图重建质量差异。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\linewidth]{figures/图 3.14 Bruceton Mine 数据集上的 MapEval 误差图.pdf}
    \caption{Bruceton Mine 数据集上的 MapEval 误差图}
    \label{fig:bruceton_mapeval_error}
\end{figure}

\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}
        \node[draw, rounded corners=4pt, thick, minimum width=13cm, minimum height=4.8cm, align=center, fill=gray!8, text width=11.3cm] {
            此处插入 Gazebo 仿真数据集的 MapEval 误差图\\[4pt]
            建议展示形式：按列排列 HDL-Graph-SLAM、LEGO\_LOAM 与 ours 的 error map，\\[2pt]
            并结合规则巷道结构观察误差在转弯和长直段中的分布。\\[4pt]
            建议与真值轨迹对比结果相互印证。
        };
    \end{tikzpicture}
    \caption{Gazebo 仿真数据集上的 MapEval 误差图}
    \label{fig:gazebo_mapeval_error}
\end{figure}

当前已完成的地图量化结果汇总于表~\ref{tab:mapeval_summary}。参考 MapEval 的指标设计，本文以 AWD 作为表征地图绝对几何一致性的核心指标，以 COM 作为覆盖完整性的补充指标，并结合 CD 与 AC 分析地图整体距离误差和点级误差水平。需要说明的是，按照当前 MapEval 实现的输出命名，结果文件中的 VMD 项对应表中的 AWD。由表中结果可知，在 Bruceton Mine 数据集上，ours 在 AWD、COM、CD 和 AC 上均优于 HDL-Graph-SLAM；在真实地下矿井数据集上，ours 在 AWD、COM、CD 和 AC 上同样整体优于 HDL-Graph-SLAM。

\begin{table}[htbp]
    \centering
    \caption{基于 MapEval 的地图量化对比结果}
    \label{tab:mapeval_summary}
    \small
    \begin{tabular}{cccccc}
        \toprule
        数据集 & 算法 & \shortstack{绝对一致性\\AWD$\downarrow$ (m)} & \shortstack{完整性\\COM$\uparrow$ (-)} & \shortstack{Chamfer 距离\\CD$\downarrow$ (m)} & \shortstack{准确性\\AC$\downarrow$ (m)} \\
        \midrule
        \multirow{2}{*}{Bruceton Mine 数据集} & HDL-Graph-SLAM & 0.567 & 0.964 & 16.353 & 0.523 \\
         & ours & \textbf{0.344} & \textbf{0.997} & \textbf{15.894} & \textbf{0.298} \\
        \multirow{2}{*}{真实地下矿井数据集} & HDL-Graph-SLAM & 1.160 & 0.954 & 30.167 & 2.920 \\
         & ours & \textbf{1.158} & \textbf{0.993} & \textbf{28.471} & \textbf{2.183} \\
        \bottomrule
    \end{tabular}
\end{table}

\subsection{基于真值轨迹的 APE 评价}

对于具有真值轨迹的 Bruceton Mine 数据集和 Gazebo 仿真数据集，本文进一步采用绝对位姿误差指标对不同方法的轨迹精度进行定量比较。其中，轨迹对比图用于直观展示参考轨迹与估计轨迹之间的偏差关系，APE 统计结果则用于补充量化误差水平。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\linewidth]{figures/图 3.17 Bruceton Mine 数据集上的轨迹对比结果-2.png}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/图 3.18 Bruceton Mine 数据集直方图-1.png}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/图 3.18 Bruceton Mine 数据集直方图-2.png}
    \end{minipage}
    \caption{Bruceton Mine 数据集绝对位置误差直方图}
    \label{fig:bruceton_ape_kde_hist}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\linewidth]{figures/图 3.19Gazebo 仿真数据集上的轨迹.png}
    \caption{Gazebo 仿真数据集上的轨迹}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/图 3.19Gazebo 仿真数据集绝对轨迹误差分布直方图-1.png}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/图 3.19Gazebo 仿真数据集绝对轨迹误差分布直方图-2.png}
    \end{minipage}
    \caption{Gazebo 仿真数据集绝对轨迹误差分布直方图}
    \label{fig:gazebo_ape_hist}
\end{figure}

表~\ref{tab:result_summary} 汇总了具有真值轨迹数据集的 APE 定量结果。由表中结果可知，在 Bruceton Mine 数据集上，Proposed 的均方根误差、平均数和中位数均明显优于对比方法；在 Gazebo 仿真数据集上，Proposed 相较于 HDL-Graph-SLAM 同样表现出更小的轨迹绝对位置误差，说明所提方法在规则巷道场景中同样具有较好的轨迹精度优势。

\begin{table}[htbp]
    \centering
    \caption{具有真值轨迹数据集的 APE 定量对比结果}
    ...
    \begin{tabular}{ccccc}
        ...
        \midrule
        \multirow{2}{*}{Bruceton Mine 数据集} & HDL-Graph-SLAM & 1.39 & 1.13 & 1.24 \\
         & Proposed & 0.25 & 0.20 & 0.20 \\
        \multirow{2}{*}{Gazebo 仿真数据集} & HDL-Graph-SLAM & 6.26 & 5.47 & 5.07 \\
         & Proposed & 2.44 & 1.96 & 1.33 \\
        \bottomrule
    \end{tabular}
\end{table}

综合当前各数据集的实验结果可以看出，本文方法在矿井巷道场景中具有较好的适用性。对于公开 Bruceton Mine 数据集，所引入的全局长度约束和全局角度约束能够有效抑制长距离运行中的误差累积，并显著提升轨迹精度，同时在 MapEval 误差图和地图量化指标上表现出更优的地图一致性；对于真实地下矿井数据集，基于参考地图的评价结果表明，所提方法能够在复杂工程环境下保持较好的地图结构一致性；Gazebo 仿真数据集则可作为补充验证场景，用于进一步分析所提几何结构约束的作用机理。
